...
Abysmal failures included first and foremost complete ignorance of when you should **not** apply linear or logistic regression and totally munging selection of the dependent variable, which I only discovered during the course of writing part 3 of this case. I will be coming back to redo the illustrative work here and in part 2 to correct that. Like the leftover in Pandora's box, revisiting this early effort illustrates the need for looking at results with a critical eye. At every step.
...
#----------------------------------------------------------------------------
# No longer needed due to new ggplot 3 features
...
#slope <- si[[1]]
#intercept <- si[[2]]
ggplot(cs) + aes(sample=fico) + stat_qq(distribution=qnorm) + stat_qq_line() + ylab("FICO") + labs(title = "QQ Plot of FICO Scores")
``` 
...
(Retrospective insight: The residuals of FICO scores against the regression line were far from normally distributed.)

### Possible strategies to deal with the issues

Sampling the pool with a reasonably large number of observations will provide a well-behaved normal distribution, courtesy of the Central Limit Theorem. (Retrospective insight: not necessarily.) Alternatively, we can cluster based on the distance of each point from the trendline.
...
d <- ggplot(cds, aes(sample=fico)) + stat_qq() + stat_qq_line() + ylab("FICO") + ggtitle("QQ Plot of FICO Scores for Dropped Loans")
d
```

The distribution reports on each deal provides the cumulative number of defaulted loans that were liquidated. These were collected manually because there were only 15 files and I anticipated no future need to examine the reports.
...
Some new fields will be important. As of the end of the first quarter 2007 *down* is the number of months that a loan has been delinquent. A value of down greater than 3 generally indicates a loan in foreclosure or a loan that has been foreclosed and is in the process of being sold. (Retrospective insight: *this* is how I should have measured performance.)
...
So, for example, if the remit field contains a value of 4198, the binary string representation, '1000001100110', can be interpreted as follows: missed the payment 12 months before, made five payments, missed two, made two, missed two, made one." (Retrospective insight: the conversion loses information before the first missed payment.)
...
The problem is thus a *classification* problem. Do FICO scores allow us to classify loans? The first task is to assign each loan its appropriate metric. (Retrospective insight: the alternative *down* parameter should have been tested, as well.)
...
(Retrospective insight: the function to classify binary digits into three categories may have been the source of the problem that I discuss at the end of part 3, which is that the distribution was not just "broadly similar," but virtually identical. I should not have written this in R, but in a purpose built parser.)
...
(Retrospective insight. The conclusion is not supported by the analysis, which will need to be reworked.)

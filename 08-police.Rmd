...
thecounted <- read.csv("data/raw/2015.csv", header = TRUE, stringsAsFactors = FALSE)
...
#save(population, file = "data/scrubbed/population.Rda")
# add lat/long to dataframe
load("data/scrubbed/coord.Rda")
counted <- bind_cols(counted, coord)
colnames(counted)[11] <- "id"
##########################################################
load("data/scrubbed/cb5.Rda")
us <- cb5
...
load("data/scrubbed/population.Rda")
# load convenience data frame cross indexing state, id, fips
load("data/scrubbed/converter.Rda")
...
datefile <- (cat(system("bin/datify | bin/datify2 | bin/stripblanks > data/raw/dates.csv", input = dateinput, intern = TRUE)))
dated <- read.csv("data/raw/dates.csv", header = FALSE, stringsAsFactors = FALSE)

...
The distributions of the three categories show the same non-normal character as the original pools.

```{r, results="asis", echo = FALSE, warning=FALSE}
si <- perf11 %>% group_by(category) %>% summarize(slope = find_intercepts(fico)[[1]], intercept  = find_intercepts(fico)[[2]])
perf11A <- perf11 %>% filter(category == 'A')
slope = si %>% filter(category == 'A') %>% select(slope)
intercept <- si %>% filter(category == 'A') %>% select(intercept)
ggplot(perf11A) + aes(sample=fico) + stat_qq(distribution=qnorm) + geom_abline(slope =  slope, intercept = intercept) +  ylab("FICO") + labs(title = "QQ Plot of Category A FICO Scores")
```

(Category B omitted pending fixing of rendering bug in RStudio)
```{r, results="asis", echo = FALSE, warning=FALSE}
perf11B <- perf11 %>% filter(category == 'B')
slope = si %>% filter(category == 'B') %>% select(slope)
intercept <- si %>% filter(category == 'B') %>% select(intercept)
p <- ggplot(perf11B) + aes(sample=fico) + stat_qq(distribution=qnorm) + geom_abline(slope =  slope, intercept = intercept) +  ylab("FICO") + labs(title = "QQ Plot of Category B FICO Scores")
```

```{r, results="asis", echo = FALSE, warning=FALSE}
perf11C <- perf11 %>% filter(category == 'C')
slope = si %>% filter(category == 'C') %>% select(slope)
intercept <- si %>% filter(category == 'C') %>% select(intercept)
ggplot(perf11C) + aes(sample=fico) + stat_qq(distribution=qnorm) + geom_abline(intercept=intercept, slope=slope) + ylab("FICO") + labs(title = "QQ Plot of Category C FICO Scores")
``` 


## Next steps

I had planned on testing the association between FICO and defaults with logistic regression, but the similarities of FICO distributions among the three categories of loan performance make that an unpromising direction. So, the next line of attack will be to select among the other variables, starting with looking at their covariance with FICO.
